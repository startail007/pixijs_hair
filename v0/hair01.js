...
var mp = new Point(600, 800);
var nodes = d3.range(0, n, 1)
    .map(function(d, i) {
        var obj = {
            p: new Point(mp.x, mp.y - i * 20),
            v: new Point(),
            weights: 0.2 * (0.3 + 0.7 * (1 - i / n)),
            len: 15 * (0.3 + 0.7 * (1 - i / n))
        };
        return obj;
    });
...
var points = [];
var ropeLength = 512 / n;
for (var i = 0; i < n; i++) {
    points.push(new PIXI.Point(i * ropeLength, 0));
}
var strip = new PIXI.mesh.Rope(PIXI.Texture.fromImage('hair01.png'), points);
stage.addChild(strip);

var hairCircle = new PIXI.Container();
stage.addChild(hairCircle);
var sprites = [];
...
d3.select("#canvas01")
    ...
    .on('click', function() {
        hairCircle.visible = !hairCircle.visible;
    })
...
function render() {
    ...
    nodes.forEach(function(d, i) {
        if (i > 0) {
            ...
            rate = Bezier(ChangeValue(NumberEx.BasicCrop((vector01.len() - nodes[i].len * 0.5) / (nodes[i].len * 1.5 - nodes[i].len * 0.5)), 0.3), 1, 0, 1);
    ...
    nodes.forEach(function(d, i) {
        if (i > 0) {
            vector01 = nodes[i].p.sub(nodes[i - 1].p);
            rate = Bezier(ChangeValue(NumberEx.BasicCrop((vector01.len() - nodes[i].len * 0.5) / (nodes[i].len * 1.5 - nodes[i].len * 0.5)), 0.3), 1, 0.5, 1);
